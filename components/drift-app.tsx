...
export default function DriftApp() {
  ...
  const [plazaMessage, setPlazaMessage] = useState("");
  const [plazaLog, setPlazaLog] = useState<string[]>([]);
  const [plazaToast, setPlazaToast] = useState("");
  const [plazaEmote, setPlazaEmote] = useState("✦");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("type-drift-state", JSON.stringify({ bottles, likedBottleIds, repliedBottleIds, nickname, profileIdentity }));
  }, [storageReady, bottles, likedBottleIds, repliedBottleIds, nickname, profileIdentity]);
  ...
  useEffect(() => { if (activePage !== "plaza") return; const api = process.env.NEXT_PUBLIC_API_URL; if (!api) return; const load = async () => { try { const response = await fetch(`${api}/api/plaza/messages`); if (!response.ok) return; const data = await response.json(); setPlazaLog((data.messages || []).map((item: { nickname?: string; body: string }) => `${item.nickname || "匿名の誰か"}「${item.body}」`)); } catch { /* オフライン時は画面内の広場を維持 */ } }; load(); const timer = window.setInterval(load, 10000); return () => window.clearInterval(timer); }, [activePage]);

  return <main className="site-shell">
    {activePage === "activity" && <section className="profile-settings"><div><p className="eyebrow">YOUR IDENTITY</p><h3>広場での呼ばれ方</h3><p>ここで設定した名前は広場とAIキャラクターとの会話だけで使われます。ボトルは匿名のままです。</p></div><input value={nickname} onChange={event => { setNickname(event.target.value); setProfileSaved(false); }} placeholder="ニックネーム（未設定なら匿名）" maxLength={24} /><div className="profile-selects"><select value={profileIdentity.mbti} onChange={event => setProfileIdentity({ ...profileIdentity, mbti: event.target.value })}><option value="">MBTI（任意）</option>{mbtiOptions.map(option => <option key={option}>{option}</option>)}</select><select value={profileIdentity.socionics} onChange={event => setProfileIdentity({ ...profileIdentity, socionics: event.target.value })}><option value="">ソシオニクス（任意）</option>{socionicsOptions.map(option => <option key={option}>{option}</option>)}</select><input value={profileIdentity.enneagram} onChange={event => setProfileIdentity({ ...profileIdentity, enneagram: event.target.value })} placeholder="エニアグラム" /></div><input value={profileIdentity.otherType} onChange={event => setProfileIdentity({ ...profileIdentity, otherType: event.target.value })} placeholder="その他の自認（自由入力）" maxLength={80} /><button type="button" className="primary-button" onClick={() => setProfileSaved(true)}>設定を保存</button>{profileSaved && <small className="profile-saved">保存しました。ボトルはこれまでどおり匿名です。</small>}</section>}
    ...
    {activePage === "plaza" && <section className="plaza-section"><div className="section-heading"><div><p className="eyebrow">TYPE PLAZA</p><h2>類型広場</h2></div><span className="online-pill">● 4人がいる</span></div><p className="plaza-lead">ここでは、ニックネームのまま過ごせます。<br />話しても、歩いても、何もしなくても。</p><div className="plaza-map"><div className="plaza-tree">🌳</div><div className="plaza-bench">🪑</div><div className="plaza-avatar avatar-you">◌<small>{nickname || "匿名のあなた"}</small>{plazaEmote && <b>{plazaEmote}</b>}</div><div className="plaza-avatar avatar-darling">🥺<small>ダーリンちゃん</small></div><div className="plaza-avatar avatar-worm">🐛<small>LSI芋虫</small></div><div className="plaza-avatar avatar-guest">◇<small>匿名の誰か</small></div><span className="plaza-bubble bubble-darling">ダーリン、ひとり？♡</span><span className="plaza-bubble bubble-worm">広場の構造を観測中。</span></div><div className="plaza-actions"><button className="primary-button" type="button" onClick={() => setPlazaToast("ひとことを流す準備ができました")}>💬 ひとこと話す</button><button className="play-button" type="button" onClick={() => { const emotes = ["✦", "🌊", "💭", "♡"]; setPlazaEmote(emotes[(emotes.indexOf(plazaEmote) + 1) % emotes.length]); }}>✦ エモート</button><button className="play-button" type="button" onClick={() => setActivePage("sea")}>🎣 海へ行く</button></div>{plazaToast && <div className="plaza-toast">{plazaToast}<button type="button" onClick={() => setPlazaToast("")}>×</button></div>}<div className="plaza-message-box"><input value={plazaMessage} onChange={event => setPlazaMessage(event.target.value)} placeholder="広場にひとこと…" maxLength={80} /><button type="button" onClick={() => { if (plazaMessage.trim()) { const message = `「${plazaMessage.trim()}」が広場に流れました`; setPlazaLog(log => [...log, message]); setPlazaToast(message); setPlazaMessage(""); } }}>送る</button></div>{plazaLog.length > 0 && <div className="plaza-log">{plazaLog.map((message, index) => <p key={`${message}-${index}`}>{message}</p>)}</div>}<div className="game-card"><div className="game-icon">🐛🟢🟢🟢</div><div><p className="eyebrow">PLAYGROUND</p><h3>芋虫浜</h3><p>🥬を食べて、他の芋虫を追い越して。<br />ひとりでもNPCと遊べる、静かなSnake系ゲーム。</p></div><button type="button" onClick={() => setActivePage("games")}>遊ぶ →</button></div><div className="plaza-mini-links"><button type="button" onClick={() => setActivePage("stars")}>✦ 認知機能の星座</button><button type="button" onClick={() => setActivePage("consult")}>☁ 自認相談室</button></div></section>}
    {activePage === "games" && <WormGame onExit={() => setActivePage("plaza")} />}
    {activePage === "stars" && <section className="activity-section stars-page"><p className="eyebrow">COGNITIVE CONSTELLATION</p><h2>認知機能の星座</h2><p className="plaza-lead">これは診断ではなく、考え方の癖を遊びながら眺める小さな観測記録です。答えによって星のつながりが変わります。</p><div className={`star-field constellation-${constellationStep % 3}`}>✦　　·　✧<br />　✧　　✦　　·<br />·　　✦　　✧</div>{constellationStep < constellationQuestions.length && <div className="constellation-question"><small>問い {constellationStep + 1} / {constellationQuestions.length}</small><h3>{constellationQuestions[constellationStep].text}</h3><div className="constellation-options">{constellationQuestions[constellationStep].options.map((option, index) => <button type="button" key={option} onClick={() => { setConstellationAnswers([...constellationAnswers, index]); setConstellationStep(step => step + 1); }}>{option}</button>)}</div></div>}{constellationStep >= constellationQuestions.length && <div className="constellation-result"><p className="eyebrow">YOUR SMALL CONSTELLATION</p><h3>{constellationAnswers.filter(answer => answer === 0).length >= 2 ? "構造を編む星座" : constellationAnswers.filter(answer => answer === 1).length >= 2 ? "遠くを読む星座" : "境界を観測する星座"}</h3><p>ここで見つけた言葉は、ボトルに流す秘密や自認相談室で考えを整理するときの入口にできます。</p><button type="button" className="play-button" onClick={() => { setConstellationStep(0); setConstellationAnswers([]); }}>もう一度観測する</button></div>}</section>}
    {activePage === "consult" && <section className="activity-section consult-page"><p className="eyebrow">TYPE CONSULTATION</p><h2>自認相談室</h2><p className="plaza-lead">自分のタイプについて迷っていることを、匿名で相談できます。<br />ここではAIは答えず、人間同士で仮説を交換します。</p><textarea className="consult-input" value={consultText} onChange={event => setConsultText(event.target.value)} placeholder="相談したいことを書く…" maxLength={1000} /><button className="primary-button" type="button" onClick={() => { if (consultText.trim()) { setConsultPosted(consultText.trim()); setConsultText(""); } }}>相談を投稿する</button>{consultPosted && <article className="consult-reply"><small>匿名の観測者から仮説</small><p>「{consultPosted}」について、まずは自分が使っている判断基準を分解して眺めてみるのはどうでしょう。ここに人間の返信が積み重なります。</p></article>}</section>}
    {activePage === "activity" && <section className="activity-section"><div className="section-heading"><div><p className="eyebrow">YOUR TRACE</p><h2>あなたの活動</h2></div><button className="primary-button" onClick={() => setComposerOpen(true)}><Plus size={18} /> 秘密を流す</button></div><div className="activity-tabs"><button className={activityTab === "mine" ? "active" : ""} onClick={() => setActivityTab("mine")}>流したボトル</button><button className={activityTab === "liked" ? "active" : ""} onClick={() => setActivityTab("liked")}>いいねしたボトル</button><button className={activityTab === "replied" ? "active" : ""} onClick={() => setActivityTab("replied")}>返信したボトル</button></div><div className="bottle-grid">{bottles.filter(b => activityTab === "mine" ? b.mine : activityTab === "liked" ? likedBottleIds.includes(b.id) : repliedBottleIds.includes(b.id)).map(bottle => <BottleCard key={bottle.id} bottle={bottle} onReact={react} onOpen={setSelected} />)}<div className="empty-mine"><Anchor size={19} /><p>ここにあなたの反応の記録が残ります。</p></div></div></section>}
